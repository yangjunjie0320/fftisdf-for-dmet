#!/bin/bash
#SBATCH --job-name=diamond-fftisdf-100-14-kmesh-6-6-7
#SBATCH --ntasks=1
...
python main.py --basis=cc-pvdz --pseudo=gth-hf-rev --kmesh=6-6-7 --density-fitting-method=fftisdf-100-14 --name=diamond --init-guess-method=minao --kconserv-to-read=/resnick/groups/changroup/members/junjiey/fftisdf-for-dmet/test/test-8-8-10/diamond-kconserv-wrap-around-1.chk
echo "End time = $(date)"
