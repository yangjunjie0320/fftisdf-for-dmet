#!/bin/bash
#SBATCH --job-name=diamond-fftisdf-100-16-kmesh-5-5-6
#SBATCH --ntasks=1
...
python main.py --basis=cc-pvdz --pseudo=gth-hf-rev --kmesh=5-5-6 --density-fitting-method=fftisdf-100-16 --name=diamond --init-guess-method=minao --kconserv-to-read=/resnick/groups/changroup/members/junjiey/fftisdf-for-dmet/test/test-8-8-10/diamond-kconserv-wrap-around-1.chk
echo "End time = $(date)"
