#!/bin/bash
#SBATCH --job-name=diamond-fftisdf-100-18-kmesh-5-5-5-lno-thresh-1.00e-06
#SBATCH --ntasks=1
...
python main.py --basis=cc-pvdz --pseudo=gth-hf-rev --kmesh=5-5-5 --density-fitting-method=fftisdf-100-18 --lno-thresh=1e-06 --name=diamond --init-guess-method=chk --df-to-read=./tmp/df.h5 --kconserv-to-read=/resnick/groups/changroup/members/junjiey/fftisdf-for-dmet/test/test-8-8-10/diamond-kconserv-wrap-around-1.chk
echo "End time = $(date)"
