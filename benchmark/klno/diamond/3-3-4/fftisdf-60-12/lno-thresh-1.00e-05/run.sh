#!/bin/bash
#SBATCH --job-name=diamond-fftisdf-60-12-kmesh-3-3-4-lno-thresh-1.00e-05
#SBATCH --ntasks=1
...
python main.py --basis=cc-pvdz --pseudo=gth-hf-rev --kmesh=3-3-4 --density-fitting-method=fftisdf-60-12 --lno-thresh=1e-05 --name=diamond --init-guess-method=chk --df-to-read=./tmp/df.h5 --kconserv-to-read=/resnick/groups/changroup/members/junjiey/fftisdf-for-dmet/test/test-8-8-10/diamond-kconserv-wrap-around-1.chk
echo "End time = $(date)"
