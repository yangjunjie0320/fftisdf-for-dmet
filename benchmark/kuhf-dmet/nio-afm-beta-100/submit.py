...
def loop(cell='diamond'):
    ...
    kmesh = ['2-2-2', '3-3-3', '4-4-4', '5-5-5', '6-6-6']
...
def main(cell='diamond', method='krhf', ntasks=1, time='00:30:00', cpus_per_task=4, reservation=None):
    ...
    for config in loop(cell=cell):
        ...
        ref_path = base_dir / "../../kuhf-dmet/nio-afm-beta-1000" / config['kmesh'] / config['density-fitting-method']
        ref_path = ref_path.resolve()
        ref_path = ref_path.absolute()
        assert ref_path.exists(), f"Reference path {ref_path} not found"

        # df_h5_path = ref_path / 'tmp' / 'df.h5'
        # assert os.path.exists(df_h5_path), df_h5_path
        ...
        config['dmet-beta'] = float(str(base_dir).split('-beta-')[1])
...
if __name__ == "__main__":
    ...
    kwargs['cell'] = pwd.name.split('-beta-')[0]
